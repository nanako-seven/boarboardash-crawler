...
browser.get("http://yue.sina.com.cn/")
localpath = "E:\\电工作业\\final\\Exercise\\sina_music.txt"
count = 0
target_num=1000
f = open(localpath, "a+")
count = 0
...
while count < target_num:
    try:
        ...
        next_page = browser.find_element_by_class_name("pagebox_next")
        informations = soup.find_all("div", attrs={
            'class': 'feed-card-item'
        })
        for element in informations:
                    url=element.find("h2").find("a").get("href")
